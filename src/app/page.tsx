...
type UpcomingMark6DrawPayload = {
  dates: string[];
  source: "website" | "fallback" | "hkjc" | "unavailable";
  hkjc?: {
    syncedAt: string;
...
export default function Home() {
  ...
  const [mark6DateSource, setMark6DateSource] = useState<UpcomingMark6DrawPayload["source"]>("fallback");
    ...
    const loadUpcomingDrawDates = async () => {
      ...
      try {
        ...
        if (active) {
          ...
          setMark6DateSource(payload.source ?? "unavailable");
        ...
        if (active) {
          ...
          setMark6DateSource("unavailable");
          setMark6HkjcSchedule(undefined);
  ...
  return (
    <Stack spacing={2.2}>
      ...
      {mark6InPlayView ? (
        <Card ref={mark6PlayViewRef} sx={{ scrollMarginTop: 96 }}>
          <CardContent>
            <Stack spacing={1.5}>
                <Button
                  size="small"
                ...
                <Box sx={{ border: "1px solid", borderColor: "divider", borderRadius: 2, p: 1.2 }}>
                  <Typography variant="body2" sx={{ mb: 0.6 }}>
                    {t.mark6HkjcDrawDayLabel}
                  </Typography>
                  {mark6HkjcSchedule?.nextDraw ? (
                    <Box sx={{ mb: 1.2, p: 1, borderRadius: 2, bgcolor: "rgba(255,213,79,0.1)", border: "1px solid", borderColor: "warning.main" }}>
                      <Typography variant="caption" color="text.secondary">
                        {t.mark6NextDrawPrizeLabel} · {mark6HkjcSchedule.nextDraw.drawDate}
                        {mark6HkjcSchedule.nextDraw.drawNo
                          ? ` · ${mark6HkjcSchedule.nextDraw.drawNo}`
                          : ""}
                      </Typography>
                      <Typography
                      ...
                      >
                        {formatMark6PrizeAmountFull(mark6HkjcSchedule.nextDraw.firstPrizeMax, locale)}
                      </Typography>
                  ...
                  {mark6HkjcSchedule?.latestResult ? (
                    <Typography variant="caption" color="text.secondary" sx={{ display: "block", mb: 1 }}>
                      ...
                      {mark6HkjcSchedule.latestResult.firstPrizePaid
                        ? formatMark6PrizeAmountFull(mark6HkjcSchedule.latestResult.firstPrizePaid, locale)
                        : formatMark6PrizeAmountFull(mark6HkjcSchedule.latestResult.firstPrizeMax, locale)}
                    </Typography>
                  ) : null}
                  {isMark6DatesLoading ? (
                  ...
                  ) : upcomingMark6Dates.length > 0 ? (
                    <Stack direction="row" spacing={0.6} useFlexGap sx={{ flexWrap: "wrap", mb: 0.8 }}>
                      {upcomingMark6Dates.map((date) => {
                        const prize =
                          mark6HkjcSchedule?.prizesByDate?.[date] ??
                        ...
                        return (
                          <Chip
                            ...
                            label={
                              prize
                                ? `${date} · ${formatMark6PrizeAmount(prize.firstPrizeMax, locale)}`
                                : date
                            }
                  ...
                  ) : mark6DateSource !== "hkjc" ? (
                    <Typography variant="caption" color="text.secondary" sx={{ display: "block", mb: 0.8 }}>
                      {t.mark6HkjcDrawDayUnavailable}
                    </Typography>
                  ) : null}
                  {mark6DatesError ? (
        ...
        <CardContent>
          <Stack spacing={2}>
            <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 1 }}>
              <Typography variant="h6">{t.mark6ResultsTitle}</Typography>
            ...
            {result && !isManualMark6 ? (
              <>
                  ...
                  <Box>
                    ...
                    {result.mode === "mark6" && baseMark6Sets.length > 0 ? (
                      <Mark6CollapsibleSection
                        id="mark6-result-advanced"
                        title={t.mark6ResultsAdvancedTitle}
                        hint={t.mark6ResultsAdvancedHint}
                        expanded={mark6ShowResultAdvanced}
                        onChange={setMark6ShowResultAdvanced}
                      >
                        <Stack spacing={0.8}>
                          <Button
                            variant="outlined"
                ...
                <Mark6CollapsibleSection
                  id="mark6-result-why"
                  title={t.mark6ResultsWhyTitle}
                  expanded={mark6ShowResultDetails}
                  onChange={setMark6ShowResultDetails}
                >
                  <Stack spacing={1}>
      ...
      {mark6ShowStandardPlayUi ? (
        <Mark6CollapsibleSection
          ...
          hint={t.mark6BackgroundSectionHint}
          expanded={mark6ShowBackground}
          onChange={setMark6ShowBackground}
        >
          <Stack spacing={1.5}>
      ...
      {mark6ShowStandardPlayUi ? (
        <Mark6CollapsibleSection
          ...
          hint={t.mark6ExploreStatsHint}
          expanded={mark6ShowAnalysis}
          onChange={setMark6ShowAnalysis}
        >
